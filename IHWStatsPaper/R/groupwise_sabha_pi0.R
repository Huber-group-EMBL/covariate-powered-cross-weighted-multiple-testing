#' SABHA multiple testing procedure for grouped side-information
#'
#' The code has been modified from https://www.stat.uchicago.edu/~rina/sabha.html to be faster in situations
#' with large m and many groups.
#'
...
inverse_sum_prox = function(Ls, y,bound){
  ...
  if(sum(Ls/y)<= bound){
    ...
    #return(uniroot_fun)
    c = stats::uniroot(uniroot_fun,c(-100,100))$root
    x = cuberoot(c+log(Ls))
  }
  x
}

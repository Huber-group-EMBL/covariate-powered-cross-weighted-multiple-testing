...
method_names <- c("BH", "Clfdr-oracle", "Clfdr-EM", "AdaPT", "IHW-BetaMix", "IHW-Grenander")
...
```{r}
betamix_r <- c(readRDS(file="../precomputed_results/betamix_simulations.Rds"),                                          readRDS(file="../precomputed_results/betamix_simulations_lfdr.Rds"))
```
...
betamix_res <-  bind_rows(unlist(betamix_r, recursive = FALSE)) %>% 
  ...
  mutate(method = fct_recode(method, "Clfdr-oracle"="Lfdr-oracle", "Clfdr-EM"="Lfdr-EM"))
```
...
null_plots <- ggplot(null_densities_df, aes(x=x, y=f, col=p)) +
                 ...
                 ylab("density") + 
                 xlab("p-value") + 
                 guides(color=guide_legend(title=expression(bar(p))))
null_plots
```
